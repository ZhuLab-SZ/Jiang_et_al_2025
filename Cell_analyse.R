...
DotPlot(
  ...
  #####################################################################################    GABAergic_neurons
  GABAergic_neurons <- subset(combined.dataset_3, idents = 0_6)
  ...
  cat("STIM：cells：", num_cells_STIM_GABAergic_neurons, "\n")
  cat("CTRL：cells：", num_cells_CTRL_GABAergic_neurons, "\n")
  ...
  fea = c("Hcn1")
  VlnPlot(GABAergic_neurons, features = fea, group.by = "group") + stat_compare_means()
  Idents(GABAergic_neurons) <- GABAergic_neurons$group
  expression_data <- FetchData(GABAergic_neurons, vars = c("Hcn1", "group"))  
  head(expression_data)  
  write.csv(expression_data, file = "E:/Hcn1-expression.csv")
  # Checking the results
  print(CD_expression)
  print(HFD_expression)
  ...
  cat("STIM：cells：", num_cells_STIM_Glutamatergic_neurons, "\n")
  cat("CTRL：cells：", num_cells_CTRL_Glutamatergic_neurons, "\n")
  ...
  write.csv(sig_markers.Astrocytes, file = "E:/sig_markers.Astrocytes_BH.csv")
  ...
  cat("STIM：cells：", num_cells_STIM_Astrocytes, "\n")
  cat("CTRL：cells：", num_cells_CTRL_Astrocytes, "\n")
  ...
  cat("STIM：cells：", num_cells_STIM_oligodendroglioma, "\n")
  cat("CTRL：cells：", num_cells_CTRL_oligodendroglioma, "\n")
  ...
  cat("STIM：cells：", num_cells_STIM_OPC, "\n")
  cat("CTRL：cells：", num_cells_CTRL_OPC, "\n")
  ...
  cat("STIM：cells：", num_cells_STIM_Epen, "\n")
  cat("CTRL：cells：", num_cells_CTRL_Epen, "\n")
  ...
  cat("STIM：cells：", num_cells_STIM_microglia, "\n")
  cat("CTRL：cells：", num_cells_CTRL_microglia, "\n")
  ...
  cat("STIM：cells：", num_cells_STIM_endothelial, "\n")
  cat("CTRL：cells：", num_cells_CTRL_endothelial, "\n")
  ...
  cat("STIM：cells", num_cells_STIM_mural, "\n")
  cat("CTRL：cells：", num_cells_CTRL_mural, "\n")
  ...
  colnames(inflammatory_gene) <- trimws(colnames(inflammatory_gene))  
  ...
  Glutamatergic_neurons <- JoinLayers(Glutamatergic_neurons)
  exprMatrix <- as.matrix(GetAssayData(Glutamatergic_neurons, assay = "RNA", layer = "data"))
  dim(exprMatrix)
  ...
  #head(geneSets)
  cells_AUC <- AUCell_calcAUC(geneSets, cells_rankings, aucMaxRank=2889)
  cells_assignment <- AUCell_exploreThresholds(cells_AUC, thrP = 0.05, plotHist = TRUE, assign=TRUE)
  print(cells_assignment)
  auc_matrix <- getAUC(cells_AUC)
  write.csv(auc_matrix, "E:/Glutamatergic_neurons.csv", row.names = FALSE)
  ################################################################################################################
  ...
  ggplot(df, aes(x = AUC)) +
    ...
    labs(title = "neuron_data_0",
         x = "AUC",
         y = "Density") +
    theme_minimal() +
    annotate("text", x = global_k1_threshold, y = 1, label = paste("Global_k1 Threshold =", round(global_k1_threshold, 4)), 
             color = "blue", angle = 90, vjust = -0.5)
  ################################################################################################################
  cluster_scores <- sapply(levels(Idents(neuron_data_0)), function(cluster) {
    cells_in_cluster <- WhichCells(neuron_data_0, idents = cluster)
